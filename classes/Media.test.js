...
describe("Media Tests", () => {
  ...
  test("Media has static ALl_MEDIA array that contains all Media instance objects", () => {
    const book = new Media("The Catcher in the Rye", 1951, "Fiction");
    expect(Media.ALL_MEDIA.length).toBe(6);
    const music = new Media("Abbey Road", 1969, "Rock");
    expect(Media.ALL_MEDIA.length).toBe(7);
  });
});
